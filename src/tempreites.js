...
;(function(root, factory) {
  ...
  function readAttributes(context, isXml) {
    function isClosingToken() {
      ...
      return (
        context.current === '>' ||
        (context.current === '/' && context.peekIgnoreWhitespace() === '>')
      )
    }
  ...
  var renderElement = function(e, datalevel) {
    ...
    if (e.sonsRefs.length) {
      for (var i = 0; i < e.sonsRefs.length; i++) {
        if (
          datalevel[e.sonsRefs[i]] &&
          typeof datalevel[e.sonsRefs[i]] === 'object'
        ) {
          if (Array.isArray(datalevel[e.sonsRefs[i]])) {
            for (var h = 0; h < datalevel[e.sonsRefs[i]].length; h++) {
              ...
              for (var l = 0; l < e.sons.length; l++) {
                ...
                renderedSons.push(
                  renderElement(son, datalevel[e.sonsRefs[i]][h])
                )
              }
            }
          } else {
    ...
    for (var i = 0; i < e.contentRefs.length; i++) {
      if (e.contentRefs[i] === '_this') {
        ...
        typeof datalevel[e.contentRefs[i]] === 'string' ||
    ...
    if (e.dataAttrRef && e.dataAttrRef.dataKey && e.dataAttrRef.htmlAttr) {
      if (
        typeof datalevel[e.dataAttrRef.dataKey] === 'string' ||
        typeof datalevel[e.dataAttrRef.dataKey] === 'number'
      ) {
        attrsDict[e.dataAttrRef.htmlAttr] = XMLdefuse(
          datalevel[e.dataAttrRef.dataKey]
        )
      }
    }
    for (var dataKey in e.attrRefs) {
      if (
        typeof datalevel[dataKey] === 'string' ||
    ...
    compile: function(template) {
      ...
      parse(template, {
        ...
        closeElement: function(tag) {
          ...
          if (element.tag !== tag) {
            throw new Error(
              'Error matching end tag with previously opened element.\n' +
                'opened element: ' +
                element.tag +
                '\n' +
                'closing tag: ' +
